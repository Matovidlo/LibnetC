...
TEST(establish_connection, test_create_ip_connection) {
    ...
    ASSERT_GE(socket, 2);
    inet_ntop(hints.ai_family, &((struct sockaddr_in *)peer)->sin_addr,
              address, peer_addr_length);
    ASSERT_EQ(address, "127.0.0.1");

    // Create TCP connection IPv6
    peer = create_ip_connection("::1", "12345", hints, &socket,
                                &peer_addr_length, true);
    inet_ntop(hints.ai_family, &((struct sockaddr_in6 *)peer)->sin6_addr,
              address, peer_addr_length);
    ASSERT_GE(socket, 2);
    ASSERT_EQ(address, "::1");
}
